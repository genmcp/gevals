...
This directory contains functional tests that exercise the `gevals` binary against mock MCP servers, agents, and LLM judges.

## Running Tests

### Subprocess Mode (Default)

Run functional tests using the compiled `gevals` binary as a subprocess:

```bash
make functional
```

This builds the `gevals` binary and runs all functional tests against it.
...
```bash
GEVALS_TEST_INPROCESS=true go test -v -tags functional ./functional/tests -run TestTaskPassesWithToolCallAndJudge
```
...
|-------------------------|-----------------------------------------|
| `GEVALS_BINARY`         | Path to gevals binary (subprocess mode) |
| `MOCK_AGENT_BINARY`     | Path to mock agent binary               |
| `GEVALS_TEST_INPROCESS` | Set to `true` to enable in-process mode |
