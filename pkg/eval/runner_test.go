...
func TestLoadMcpConfig(t *testing.T) {
	...
	clearEnv := func() {
		envVars := []string{
			...
			mcpproxy.EnvMcpCommand, mcpproxy.EnvMcpArgs, mcpproxy.EnvMcpServerName,
	...
	tests := map[string]struct {
		...
		"config file takes priority over env vars": {
			...
			validateFunc: func(t *testing.T, config *mcpproxy.MCPConfig) {
				...
				_, hasDefault := config.MCPServers["kubernetes"]
		...
		"stdio server from env vars": {
			...
			validateFunc: func(t *testing.T, config *mcpproxy.MCPConfig) {
				require.NotNil(t, config)
				server, hasServer := config.MCPServers["kubernetes"]
